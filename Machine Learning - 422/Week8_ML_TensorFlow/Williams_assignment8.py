"""
Created on Thu Nov  9 10:06:21 2017

Submitted by: Tamara Williams

Neural network material based on examples from the book: 
"Hands-on Machine Learning with Scikit-Learn and TensorFlow" (chapter 10)
by Aurélien Geron

All code between the ++++++++++++ delineators was provided by Thomas W. Miller
based on his book "Web and Network Data Science Modeling Techniques in 
Predictive Analytics"  specifically the chapter on measuring sentiment
A single change was made to the instructor's code, see lines 184 & 185

Student additions begin at line 858
"""
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#
...
# Revised for Python 3.x  by Thomas W. Miller 2017/11/09
...
def corpus_creator (input_directory_path, output_directory_path):
    ...
    #use os.makedirs with exist_ok to handle the directory exists case
    os.makedirs(parsed_file_directory, exist_ok=True)  
...
# define word lists to be used in subsequent modeling
# these come from previous analyses of word frequencies
# across the corpus comparing them with positive/negative lists
# 
# these lists correspond to lists used in user-defined scoring 
# functions under the python_utilities.py module
selected_positive_words = ['beautiful','best','better',\
        'classic', 'enjoy', 'enough',\
        'entertaining', 'excellent',\
        'fans', 'fun', 'good', 'great',\
        'interesting', 'like', 'love', 'nice',\
        'perfect', 'pretty', 'right', 'top',\
        'well', 'won', 'wonderful', 'work',\
        'worth']
print('\nSelected Positive Words:', selected_positive_words)
   
selected_negative_words = ['bad', 'boring', 'creepy',\
        'dark', 'dead', 'death', 'evil', 'fear',\
        'funny','hard', 'kill', 'killed', 'lack',\
        'lost', 'mystery', 'plot','poor',\
        'problem', 'sad', 'scary', 'slow',\
        'terrible', 'waste', 'worst', 'wrong']
...
print('\n--------------------------------------------------')
print('Regression Difference Training Set Performance\n',\
    'Percentage of Reviews Correctly Classified:',\
...
print('\n--------------------------------------------------')
print('Regression Difference Test Set Performance\n',\
    'Percentage of Reviews Correctly Classified:',\
...
# --------------------------------------------
print('\n--------------------------------------------------')
print('Text Measures for Each Corpus')
...
print('\n test_data_frame (rows, cols):',test_data_frame.shape,'\n')
print(test_data_frame.describe())
print(test_data_frame.head())


print('\nCreate tom_data_frame')
...
# examine the expanded testing data frame
print('\n tom_data_frame (rows, cols):',tom_data_frame.shape,'\n')
...
print('\n--------------------------------------------------')
print('Word/item Analysis Method for Training Set')
...
# --------------------------------------
print('\n--------------------------------------------------')
print('Logistic Regression by Scikit Learn')
...
x_train = train_data_frame[item_list]
y_train = train_data_frame['y']

# define the logistic regression algorithm 
my_logit_model = LogisticRegression(random_state = 9999)
my_logit_model_fit = my_logit_model.fit(y = y_train, 
                                        X = x_train)
print('\nModel specification: ', my_logit_model_fit.get_params())
print('\nFitted coefficients: ', my_logit_model_fit.coef_)

# predicted probability of thumbs up for training set
train_data_frame['pred_logit_prob'] =\
    my_logit_model.predict_proba(X = x_train)[:,1]
...
x_test = test_data_frame[item_list]
...
test_data_frame['pred_logit_prob'] =\
    my_logit_model.predict_proba(X = x_test)[:,1]
...
# --------------------------------------
print('\n--------------------------------------------------')
print('Support Vector Machines a la Miller (2015)')

my_svm = svm.SVC()  
my_svm_fit = my_svm.fit(X = x_train, y = y_train)
train_data_frame['pred_svm_binary'] = my_svm_fit.predict(X = x_train)
...
# thumbs up or down reviews in the test set 
# assume that y is not known... only x used from patsy
test_data_frame['pred_svm_binary'] = my_svm_fit.predict(X = x_test)
...
# --------------------------------------
print('\n--------------------------------------------------')
print('Random Forests a la Miller (2015)')
...
my_rf_model_fit = my_rf_model.fit(X = x_train, y = y_train)
train_data_frame['pred_rf_binary'] = my_rf_model_fit.predict(X = x_train)
...
# thumbs up or down reviews in the test set 
# assume that y is not known... only x used from patsy
test_data_frame['pred_rf_binary'] = my_rf_model_fit.predict(X = x_test)
...
# suppress all the tensorflow messages, after the 3rd time thru they are annoying
# 
...
#dnn_clf = tf.contrib.learn.DNNClassifier(hidden_units=[n_hidden1,n_hidden2, 
#                                n_hidden3, n_hidden4, n_hidden5], n_classes=2,
#                                feature_columns=feature_cols, config=config)
dnn_clf = tf.contrib.learn.DNNClassifier(hidden_units=[n_hidden1,n_hidden2], 
                    n_classes=2, feature_columns=feature_cols, config=config)

dnn_clf = tf.contrib.learn.SKCompat(dnn_clf) # if TensorFlow >= 1.1
dnn_clf.fit(x_train, y_train, batch_size=batch_size, steps=10000)
...
train_data_frame['tf_pred_rf_binary'] = y_pred_train['classes']
train_data_frame['tf_pred_rf'] = train_data_frame['tf_pred_rf_binary'].map(binary_to_thumbsupdown)

print('\n--------------------------------------------------')
print('\n Neural Network Training Set Performance\n',\
    'Percentage of Reviews Correctly Classified:',\
    100 * round(evaluate_classifier(train_data_frame['tf_pred_rf'],train_data_frame['thumbsupdown'])[4], 3),'\n')

# use the model developed on the training set to predict
# thumbs up or down reviews in the test set 
# assume that y is not known... only x used from patsy
test_data_frame['tf_pred_rf_binary'] = y_pred_test['classes']
test_data_frame['tf_pred_rf'] = test_data_frame['tf_pred_rf_binary'].map(binary_to_thumbsupdown)


print('\n Neural Network Test Set Performance\n',\
    'Percentage of Reviews Correctly Classified:',\
    100 * round(evaluate_classifier(test_data_frame['tf_pred_rf'],\
    test_data_frame['thumbsupdown'])[4], 3),'\n')

#print graphical version of results 
conf_mat=confusion_matrix(test_data_frame['pred_rf_binary'], y_pred_test['classes'])
...
chart_name = "Actual versus Predicted"
save_name = "TF_conf.pdf"
plt.title(chart_name)
plt.show()

print('\n--------------------------------')

## make a more elborate model for finer-grained tuning
#------------------------------------------------------------------------------
config = tf.contrib.learn.RunConfig(tf_random_seed=13)
feature_cols = tf.contrib.learn.infer_real_valued_columns_from_input(x_train)
...
dnn_clf2 = tf.contrib.learn.DNNClassifier(hidden_units=[n_hidden1,n_hidden2],
            n_classes=2, 
...
dnn_clf2 = tf.contrib.learn.SKCompat(dnn_clf2) # if TensorFlow >= 1.1
dnn_clf2.fit(x_train, y_train, batch_size=batch_size, steps=10000)

# get the predictions
y_pred_train = dnn_clf2.predict(x_train)
y_pred_test = dnn_clf2.predict(x_test)
...
print('\n--------------------------------------------------')
print('\n Neural Network Training Set Performance, Tuned network\n',\
    'Percentage of Reviews Correctly Classified:',\
    100 * round(evaluate_classifier(train_data_frame['tf_pred_rf'],train_data_frame['thumbsupdown'])[4], 3),'\n')
...
print('\n')
print('pass using AdagradOptimizer')
#print('pass using AdamOptimizer')
print('\n Neural Network Test Set Performance\n',\
    'Percentage of Reviews Correctly Classified:',\
    100 * round(evaluate_classifier(test_data_frame['tf_pred_rf'],\
    test_data_frame['thumbsupdown'])[4], 3),'\n')
